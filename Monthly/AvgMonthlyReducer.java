...
public class AvgMonthlyReducer extends MapReduceBase implements Reducer<Text, Text, Text, Text>{
	...
	public void reduce(Text key, Iterator<Text> values,
		...
		if (tempind > 1 ){
			if ( avgTemp > temps.get(tempind - 2)){
				tempresults = "Hotter than previous month";
			}
			else if (avgTemp < temps.get(tempind - 2)){
				tempresults = "Colder than previous month";
			}
			else {
				tempresults = "Temperature same as previous month";
			}
		}
		if (precpind > 1 ){
			if ( avgPrecp > precps.get(precpind - 2)){
				precpresults = "Precipitation more than previous month";
			}
			else if (avgPrecp < precps.get(precpind - 2)){
				precpresults = "Precipitation less than previous month";
			}
			else {
				precpresults = "Precipitation same as previous month";
			}
		}
		if (windind > 1 ){
			if ( avgWind > winds.get(windind - 2)){
				windresults = "Wind speed more than previous month";
			}
			else if (avgWind < winds.get(windind - 2)){
				windresults = "Wind speed less than previous month";
			}
			else {
				windresults = "Wind speed same as previous month";
			}
		}
