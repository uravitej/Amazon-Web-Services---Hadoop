...
public class AvgSeasonReducer extends MapReduceBase implements Reducer<Text, Text, Text, Text>{
	...
	public void reduce(Text key, Iterator<Text> values,
		...
		if (tempind > 1 ){
			if ( avgTemp > temps.get(tempind - 2)){
				tempresults = "Hotter than previous season";
			}
			else if (avgTemp < temps.get(tempind - 2)){
				tempresults = "Colder than previous season";
			}
			else {
				tempresults = "Temperature same as previous season";
			}
		}
		if (precpind > 1 ){
			if ( avgPrecp > precps.get(precpind - 2)){
				precpresults = "Precipitation more than previous season";
			}
			else if (avgPrecp < precps.get(precpind - 2)){
				precpresults = "Precipitation less than previous season";
			}
			else {
				precpresults = "Precipitation same as previous season";
			}
		}
		if (windind > 1 ){
			if ( avgWind > winds.get(windind - 2)){
				windresults = "Wind speed more than previous season";
			}
			else if (avgWind < winds.get(windind - 2)){
				windresults = "Wind speed less than previous season";
			}
			else {
				windresults = "Wind speed same as previous season";
			}
		}
